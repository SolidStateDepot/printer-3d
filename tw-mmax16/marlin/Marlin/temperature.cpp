...
void manage_heater()
{
  ...
  if (isHeatingHotend(e)) {
      int dh = degHotend(e);
      if ((long)(millis() - watchmillis) > WATCHPERIOD) {
        if (dh - watch_oldtemp[e] < WATCHPERIOD_HYSTERESIS) {
          setTargetHotend(0, e);
          LCD_MESSAGEPGM("Thermistor failed - extruder set to 0");
          SERIAL_ECHO_START;
          SERIAL_ECHOLN("Thermistor failed - extruder set to 0");
        }
        watch_oldtemp[e] = max(0, dh);
        watchmillis = millis();
      }
    } else {
      watchmillis = millis();
